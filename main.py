...
if config.Preprocessor:
    ...
    if config.Preprocessor_calc_mov_avg:
        ...
        solar_pv_power_CFR_moving_avg_h.to_csv(
            'solar_pv_power_CFR_moving_avg_h.csv', sep=';', encoding='latin1', index=False)
...
#            config.file_path_ext_ssd + 'wind_power_offs_moving_avg.csv', sep=';', encoding='latin1', index=False)
    ...
    if config.Preprocessor_read_data_mov_avg:
        solar_pv_power_CFR_moving_avg = pd.read_csv('solar_pv_power_CFR_moving_avg_h.csv', error_bad_lines=False,
    ...
    if config.Preprocessor_installed_capacity_corrector:
        ...
        installed_capacity_factor_solar_pv_power.to_csv(
            'installed_capacity_factor_solar_pv_power_h.csv', sep=';', encoding='latin1', index=False)
    ...
    installed_capacity_factor_solar_pv_power = pd.read_csv('installed_capacity_factor_solar_pv_power_h.csv', error_bad_lines=False, sep=';', encoding = 'latin1', index_col= False, low_memory=False)
    ...
    # dunkelflaute_freq_country_i_th02 = Preprocessor.FrequencyCalculatorCFRBelowThresholdOneEnergyVariableOneThresholds(
    #     installed_capacity_factor_wind_power_offs, 'DE', 0.2, 'wind_power_offs')
    ...
    dunkelflaute_freq_country_i = Preprocessor.FrequencyCalculatorCFRBelowThreshold(installed_capacity_factor_solar_pv_power, installed_capacity_factor_wind_power_ons, installed_capacity_factor_wind_power_offs, 'DE', [0.9, 0.3, 0.3])
