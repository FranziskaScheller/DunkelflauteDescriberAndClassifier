...
for year_i in res_eval_df_probs_rfc[res_eval_df_probs_rfc['DF_ind'] == 1].Date.apply(lambda x: x.year).unique()[1:]:
    ...
    ax[j, k].scatter(data_all['DF_pred'],
                data_all['wind_ons_DE'], s=3, color='teal', label=str(year_i))
...
fig.delaxes(ax[1, 6])
fig.supxlabel('Predicted probability of onshore wind capacity factor <= 0.25 from Balanced Random Forest classifier', fontsize=19)
fig.supylabel('Onshore wind capacity factor', fontsize=19)
plt.savefig(
...
for year_i in res_eval_df_probs_rfc[res_eval_df_probs_rfc['DF_ind'] == 1].Date.apply(lambda x: x.year).unique():
    ...
    ax[0].plot(data_wind_year_i['Date'], data_wind_year_i['DE'], color='navy', label='Onshore wind CF', zorder=-1)
    ...
    ax[1].plot(data_year_i['Date'], data_year_i['DF_pred'], color='teal', label='Probability BRF Classifier (onshore wind)', zorder=-1)
    ax[1].scatter(data_year_i_df['Date'], data_year_i_df['DF_pred'], s=7, color='crimson', label='Low onshore wind CF', zorder=1)
